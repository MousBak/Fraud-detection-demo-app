...
async def simulate_l2d(request: L2DSimulationRequest):
    ...
    try:
        ...
        try:
            ...
            np.random.seed(42)
            ml_scores = np.clip(np.random.beta(2, 5, len(y_true)), 0.01, 0.99)
            ml_scores[y_true == 1] = np.clip(ml_scores[y_true == 1] + 0.4, 0.01, 0.99)
            ml_predictions = (ml_scores > 0.5).astype(int)

        # Préparer experts
        ensemble = ExpertEnsemble()
        ensemble.load_predictions(expert_preds)
        expert_consensus = ensemble.get_consensus()

        # Simuler L2D
        l2d = L2DSystem()
        l2d.setup(ml_scores, ml_predictions, expert_consensus, y_true)
